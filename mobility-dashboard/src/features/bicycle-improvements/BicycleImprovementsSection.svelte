<script lang="ts">
  ...
  import {
    ...
    type BicycleImprovementDataset,
  } from "../../data/bicycleImprovements";
  import {
    dashboardFilters,
  ...
  type BicycleImprovementChartRow = {
    topic: string;
    topic_label: string;
    topic_order: number;
  ...
  onMount(async () => {
    try {
      dataset = await loadBicycleImprovementData();
    } catch (loadError) {
  ...
  let visibleValues = $derived.by(() => {
    ...
    const topicRows = new Map<string, BicycleImprovementChartRow[]>();
    ...
    for (const topicMeta of fixedTopicOrder) {
      ...
      const chartRows = validEntries.map((row) => {
        ...
        } satisfies BicycleImprovementChartRow;
      });
